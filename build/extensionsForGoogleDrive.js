...
exports.driveX = void 0;

var _searchStringForGoogleDrive = _interopRequireDefault(require("search-string-for-google-drive"));
...
function extensionsForGoogleDrive({
  ...
  function driveSearcher(options) {
    ...
    const searchTerms = _searchStringForGoogleDrive.default.extract(options);
    ...
    return async function (parent, name) {
      ...
      const searchString = (0, _searchStringForGoogleDrive.default)(search, allowMatchAllFiles);
      const params = {
        spaces,
        q: searchString,
        pageSize: limit,
  ...
  function driveJanitor(fileListProperty, successProperty) {
    ...
    return async function (info) {
      ...
      if (Array.isArray(files) && files.length > 0) {
        return Promise.all(files.map(deleteFile)).then(() => {
  ...
  x.stepRight = driveStepRight; // see https://developers.google.com/drive/v3/web/folder
  ...
  function driveFolderCreator(meta) {
    return async function (f, name) {
      ...
      fieldList.unshift('id'); // see https://stackoverflow.com/questions/34905363/create-file-with-google-drive-api-v3-javascript
  ...
  function driveFolderFactory(meta) {
    ...
    return async function (f, name) {
      ...
      try {
        ...
        if (e.toString().includes(FileNotFound)) folder = await creator(f, name);else throw e;
      }
  ...
  async function driveFindPath(path, fields) {
    ...
    const metadata = await (0, _pReduce.default)(parts, stepper, rootFolderId);
    return metadata;
  }
  ...
  async function driveCreatePath(path, meta) {
    ...
    const parentFolder = await (0, _pReduce.default)(parts, dff, rootFolderId);
...
const driveX = extensionsForGoogleDrive({
  rootFolderId: 'root',
  spaces: 'drive'
});
exports.driveX = driveX;
